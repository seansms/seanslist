...
###########################################################################
# Class NewListFrameOver a popup form over SLFrame1
###########################################################################
import wx
import wx.xrc
...
class NewListFrameOver(NewListFrame):

    def __init__(self, parent):
        self.parent = parent
        NewListFrame.__init__(self, parent)

    def __del__(self):
        pass

    def on_click_add_newlist( self, event ):
        list_name = self.m_textCtrl_NewListName.GetValue()
        self.parent.re_save_mylists_plus_new(self.parent.ctrls, list_name)
        self.Hide()
